...
app.post("/api/files", async (c) => {
  ...
  const account = c.env.VITE_R2_ACCOUNT_ID || "";
  const bucket = c.env.VITE_R2_BUCKET_NAME || "";
  const publicUrl = account && bucket ? `https://${account}.r2.cloudflarestorage.com/${bucket}/${encodeURIComponent(key)}` : `https://r2-placeholder.invalid/${encodeURIComponent(key)}`;
  const result = await c.env.DB.prepare(`
...
app.get("/api/stats", async (c) => {
  ...
  if (results) {
    for (const r of results) {
      if (r.is_guest === 1)
        guestBytes = Number(r.total_bytes || 0);
      else
        userBytes = Number(r.total_bytes || 0);
    }
  }
